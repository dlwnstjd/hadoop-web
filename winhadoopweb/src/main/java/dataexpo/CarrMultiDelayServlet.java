...
public class CarrMultiDelayServlet extends HttpServlet{
	private static final long serialVersionUID = 1L;
	public CarrMultiDelayServlet () {
		super();
	}
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		String output = req.getSession().getServletContext().getRealPath("/") + "/carrmultiout" + year;
		System.out.println(input);
		System.out.println(output);
		Configuration conf = new Configuration();
		try {
			Job job = new Job(conf, "CarrMultiDelayServlet");
			FileInputFormat.addInputPath(job, new Path(input));
			FileOutputFormat.setOutputPath(job, new Path(output));
			job.setJarByClass(CarrMultiDelayServlet.class); // getClass() : Object 클래스의 멤버 (해당 객체의 클래스 정보를 리턴)
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/dataexpo/dataexpo4.jsp");
